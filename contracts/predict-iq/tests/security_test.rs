...
mod test_security {
    ...
    use predict_iq::types::OracleConfig;
    use predict_iq::errors::ErrorCode;

    fn setup_test() -> (Env, PredictIQClient<'static>, Address, token::StellarAssetClient<'static>) {
        let e = Env::default();
        e.mock_all_auths();
    ...
    fn test_oracle_manipulation_prevention() {
        let (e, client, user, token_client) = setup_test();
    ...
    #[test]
    fn test_reentrancy_protection_place_bet() {
        let (e, client, user, token_client) = setup_test();
